...
app.get("/", (req, res) => {
    res.send("All is good at main view server")
})
...
function GetTotal(collectionName, query, resolve) {
    ...
    collectionName.find(query).toArray()
    .then((result) => {
        let fare_array = []
        const Sum = arr => arr.reduce((num1,num2) => num1+num2, 0)
        result.map((ride) => {fare_array.push(Number(ride['fare']))})
...
function GetDailyRegistered(collectionName, resolve) {
    ...
    collectionName.find({date_registered: { "$gte": startOfToday }}).toArray()
    .then((result) => {
...
function GetCashWallet(collectionName, resolve) {
    ...
    collectionName.find({isArrivedToDestination: true}).toArray()
    .then((result) => {
        let fare_array = []
        let fare_array_cash = []
...
clientMongo.connect(function (err) {
    ...
    new Promise((res) => {
        GetTotal(collectionRidesDeliveryData,{ isArrivedToDestination: true}, res)
    })
    .then((result) => {
        console.log(result)
        new Promise((res) => {
        ...
        .then((result2) => {
            ...
            new Promise((res) => {
                GetTotal(collectionRidesDeliveryData,{
                    date_requested: {$regex: new Date().toISOString().replace(/\T.*/, ' '), $options:'i'},
                    isArrivedToDestination: true}, res)
            })
            .then((result3) => {
                console.log(result3)
                ...
                new Promise((res) => {
                    GetTotal(collectionRidesDeliveryDataCancelled,{
                        date_requested: {$regex: new Date().toISOString().replace(/\T.*/, ' '), $options:'i'}}, res)
                })
                .then((result4) => {
                    console.log(result4)

                    Promise.all([
                        new Promise( (res) => {GetDailyRegistered(collectionDrivers_profiles,res)}),
                        new Promise( (res) => {GetDailyRegistered(collectionPassengers_profiles, res)}),
                        new Promise( (res) => {GetCashWallet(collectionRidesDeliveryData, res)})
                    ])
                    .then((data) => {
                        ...
                        finalObject.totalTripSuccessfulToday = result3.total_rides
                        finalObject.totalFareCancelledToday = result4.total_fare
                        finalObject.totalTripCancelledToday = result4.total_rides
                        finalObject.totalNewDriverToday = dataDriver.totalRegisteredToday
                        finalObject.totalNewPassengerToday = dataPassenger.totalRegisteredToday
                        finalObject.totalCash = dataCashWallet.totalCash
                        finalObject.totalWallet = dataCashWallet.totalWallet

                        /* console.log(finalObject)
                        //console.log(dataCashWallet)
