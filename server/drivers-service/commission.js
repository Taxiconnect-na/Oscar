...
clientMongo.connect(function () {
  ...
  //! Variables
  const recipient_fp =
    "xx xx xe03653169b9c18193a0b8dd329ea1e43eb0626ef9f16b5b979694a429710561a3cb3ddae".replace(
      /\s/g,
      ""
    );
  ...
  new Promise((res) => {
    MakePaymentCommissionTCSubtracted(
    ...
    .then((outcome) => {
      logger.info(outcome);
      if (outcome.success) {
        logger.info("SUCCESSFUL INSERTION OF DRIVER PAYMENT (COMMISSION)");
        // Send back successful response object
      } else if (outcome.error) {
        logger.info("FAILED TO INSERT PAYMENT");
