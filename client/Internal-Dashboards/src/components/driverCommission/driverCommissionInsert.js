...
const DriverRow2 = (props) => {
  return (
    <tr>
      <td style={{ fontSize: "large", paddingBottom: "4%" }}>
        {" "}
        {props.dataWallet.month}
      </td>
...
export default function DriverCommissionInsert({ location }) {
  ...
  let [drivers_data_cash, setDriverDataCash] = useState([
    { total_cash: "", month: "" },
  ]);
  let [drivers_data_wallet, setDriverDataWallet] = useState([
    { total_wallet: "", month: "" },
  ]);

  useEffect(() => {
    const { driver_identifier, taxi, dname, dsurname } = queryString.parse(
      location.search
    );
  ...
  const onSubmitHandler = (event) => {
    ...
    const options = {
      ...
      body: JSON.stringify({
        amount: amount,
        driver_fingerprint: queryString.parse(location.search)
          .driver_identifier,
      }),
    };
