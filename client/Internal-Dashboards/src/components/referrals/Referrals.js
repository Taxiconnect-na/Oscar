...
const ActiveRow = (props) => {
  ...
  const PaymentHandler = () => {
    ...
    fetch(
      `${process.env.REACT_APP_DRIVER_SERVER}/update-referral-paid-status/${props.data.referral_fingerprint}`
    )
      .then((data) => data.json())
      .then((response) => {
        if (response.success) {
  ...
  const RejectionHandler = () => {
    ...
    fetch(
      `${process.env.REACT_APP_DRIVER_SERVER}/update-referral-rejection-status/${props.data.referral_fingerprint}`
    )
      .then((data) => data.json())
      .then((response) => {
        if (response.success) {
  ...
  const DeleteReferralUserSideHandler = () => {
    ...
    fetch(
      `${process.env.REACT_APP_DRIVER_SERVER}/mark-referral-deleted-user-side/${props.data.referral_fingerprint}`
    )
      .then((data) => data.json())
      .then((response) => {
        if (response.success) {
  ...
  const DeleteReferral = () => {
    ...
    fetch(
      `${process.env.REACT_APP_DRIVER_SERVER}/delete-referral/${props.data.referral_fingerprint}`
    )
      .then((data) => data.json())
      .then((response) => {
        if (response.success) {
  ...
  return (
    <>
      <tr>
        <td>
          {props.data.date_referred.toString().slice(0, 10)} |{" "}
          {props.data.date_referred.toString().slice(11, 19)}
        </td>
        <td>{props.data.driver_phone}</td>
        <td>{props.data.taxi_number}</td>
        <td>
          {props.data.expiration_time.slice(0, 10)} |{" "}
          {props.data.expiration_time.slice(11, 19)}
        </td>
        <td>{props.data.driver_name}</td>
        <td>{props.data.is_paid ? "Yes" : "No"}</td>
        <td>
          <VscTriangleDown onClick={() => setDetails(!details)} />{" "}
        </td>
      </tr>
        ...
        <table style={{ width: "100%" }} className="table-bordered">
          ...
          <tbody>
            <tr>
              ...
              <td rowSpan={4}>
                <div
                  style={PaymentHideStyle}
                  className="active-referrals-action"
                >
                  <div>
                    <button
                      className="btn btn-success btn-sm"
                      onClick={() => showPaymentOption()}
                      style={hidePaymentAskStyle}
                    >
                  ...
                  <div style={rejectedHideStyle}>
                    <button
                      className="btn btn-success btn-sm"
                      onClick={() => showRejectionOption()}
                      style={hideRejectionAskStyle}
                    >
                      {props.data.is_referral_rejected
                        ? "Mark unrejected"
                        : "Mark rejected"}
                    </button>
                    ...
                    <div style={hideRejectionButtonStyle}>
                      ...
                      <button
                        className="btn btn-info btn-sm"
                        onClick={() => showAllActionsRejection()}
                        style={{ margin: "5%" }}
                      >
                        NO
                      </button>
                  ...
                  <div style={deleteUserSideStyle}>
                    <button
                      ...
                      onClick={() => showDeleteUserSideOption()}
                      style={hideDeleteUserSideAskStyle}
                    >
                      {"Delete from rider"}
                    </button>
                    ...
                    <div style={hideDeleteUserSideButtonStyle}>
                      <h5>
                        {" "}
                        Would you like to delete referral from rider side ?
                      </h5>
                      ...
                      <button
                        className="btn btn-info btn-sm"
                        onClick={() => showAllActionsDeleteUserSide()}
                        style={{ margin: "5%" }}
                      >
                        NO
                      </button>
                  ...
                  <div style={deleteReferralStyle}>
                    <button
                      className="btn btn-warning btn-sm"
                      onClick={() => showDeleteReferralOption()}
                      style={hideDeleteReferralAskStyle}
                    >
                      {"Delete referral"}
                    </button>
                    ...
                    <div style={hideDeleteReferralButtonStyle}>
                      <h5>
                        {" "}
                        Would you like to completely delete the referral ?
                      </h5>
                      ...
                      <button
                        className="btn btn-info btn-sm"
                        onClick={() => showAllActionsDeleteReferral()}
                        style={{ margin: "5%" }}
                      >
                        NO
                      </button>
                  ...
                  <div style={registerDriverStyle}>
                    <button
                      className="btn btn-info btn-sm"
                      onClick={() => registerReferredDriver()}
                    >
...
export default function Referrals() {
  ...
  const RowDataActive = () => {
    if (active_referrals.length > 0) {
      ...
      return (
        <td colSpan={7} style={no_data_style}>
          {" "}
          <h1>No active referral available</h1>{" "}
        </td>
      );
    }
  };
  ...
  const RowDataExpired = () => {
    if (expired_referrals.length > 0) {
      ...
      return (
        <td colSpan={7} style={no_data_style}>
          {" "}
          <h1>No expired referral available</h1>{" "}
        </td>
      );
    }
  };
  ...
  const RowDataRejected = () => {
    if (rejected_referrals.length > 0) {
      ...
      return (
        <td colSpan={7} style={no_data_style}>
          {" "}
          <h1>No rejected referral available</h1>{" "}
        </td>
      );
    }
  };
