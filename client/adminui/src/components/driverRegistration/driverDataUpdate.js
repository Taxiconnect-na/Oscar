...
export default function DriverDataUpdate({ location }) {
    ...
    const onSubmitDriverInfo = async (e) => {
        ...
        try {
            ...
            const data = await fetch(`${process.env.REACT_APP_GATEWAY}/update-driver-info`, options)
    ...
    // Taxi picture
    if(uploading) {
        state = <h5 style= { state_style.uploading} className="uploading"> Uploading Taxi picture...</h5>
    }
    if(success) {
    ...
    if(uploading_profile) {
        state_profile = <h5 style= { state_style.uploading} className="uploading"> Uploading profile picture...</h5>
    }
    if(success_profile) {
        state_profile = <h5 style={ state_style.success } className="upload-success"> Success</h5> 
    } 
    if(failure_profile) {
        state_profile = <h5 style={ state_style.failure} className="upload-failure"> Failed </h5>
    }

    // Driver Licence
    if(uploading_licence) {
        state_licence = <h5 style= { state_style.uploading} className="uploading"> Uploading driver's licence...</h5>
    }
    if(success_licence) {
        state_licence = <h5 style={ state_style.success } className="upload-success"> Success</h5> 
    } 
    if(failure_licence) {
        state_licence = <h5 style={ state_style.failure} className="upload-failure"> Failed </h5>
    }

    // id paper
    if(uploading_id) {
        state_id = <h5 style= { state_style.uploading} className="uploading"> Uploading Id...</h5>
    }
    if(success_id) {
        state_id = <h5 style={ state_style.success } className="upload-success"> Success</h5> 
    } 
    if(failure_id) {
        state_id = <h5 style={ state_style.failure} className="upload-failure"> Failed </h5>
    }

    // white paper
    if(uploading_white) {
        state_white = <h5 style= { state_style.uploading} className="uploading"> Uploading white_paper...</h5>
    }
    if(success_white) {
        state_white = <h5 style={ state_style.success } className="upload-success"> Success</h5> 
    } 
    if(failure_white) {
        state_white = <h5 style={ state_style.failure} className="upload-failure"> Failed </h5>
    }

    // public permit
    if(uploading_permit) {
        state_permit = <h5 style= { state_style.uploading} className="uploading"> Uploading public permit...</h5>
    }
    if(success_permit) {
        state_permit = <h5 style={ state_style.success } className="upload-success"> Success</h5> 
    } 
    if(failure_permit) {
        state_permit = <h5 style={ state_style.failure} className="upload-failure"> Failed </h5>
    }

    // Blue Paper
    if(uploading_blue) {
        state_blue = <h5 style= { state_style.uploading} className="uploading"> Uploading blue paper...</h5>
    }
    if(success_blue) {
        state_blue = <h5 style={ state_style.success } className="upload-success"> Success</h5> 
    } 
    if(failure_blue) {
        state_blue = <h5 style={ state_style.failure} className="upload-failure"> Failed </h5>
    }

    // For
    let info_update_result
    if(success_info) {
        info_update_result = <AiOutlineCheckSquare style={{width: 30, height: 30, color: "green", marginLeft:"5%"}} />
    }
    if(uploadinginfo) {
        info_update_result = <h5 className="uploading-info-anime">In progress...</h5>
    }
    if(failure_info) {
        info_update_result = <VscError style={{width: 30, height: 30, color: "red", marginLeft:"5%"}} />
    }
  ...
  return (
    ...
    <div className="template">
        ...
        <div className="main-content" >
            ...
            <h2 style={{ display: "grid", placeItems:"center", padding:"2%"}}> DRIVER PROFILE </h2>
                {/* 
                ...
                */}
                <div className="profile-info-driver">
                    ...
                    <div>
                        <form style={formStyle} onSubmit={onSubmitDriverInfo}>
                            ...
                            <div>
                                <label> Plate number </label><br></br>
                                <input type="text"
                                    className="my-input-field"
                                    value={ plate_number }
                                    disabled={info_form_state}
                            ...
                            <div style={{ display: "inline-block", width: "100%"}}>
                                ...
                                <button disabled={info_form_state} className="btn btn-info btn-sm" type="submit">Update</button>
                                { info_update_result }
                            </div>    
                        </form>
                        <button style={{marginLeft: "80%"}} onClick={formControlDisplay} className="btn btn-warning btn-sm">
                ...
                <div style={{ display:"grid", placeItems:"center", marginTop: "4%"}}>
                    ...
                    <h4 style={{ backgroundColor:"#0a0321", padding:"2%", color: "white"}}> UPLOAD / UPDATE  FILES</h4>
                    ...
                    <div className="custom-file mt-4" >
                            ...
                            <label className="custom-file-label" htmlFor="customFile" style={{ overflow: "hidden"}}>
                                {taxi_picture_name}
                            </label> 
                    </div>
